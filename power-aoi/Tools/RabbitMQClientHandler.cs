...
namespace power_aoi.Tools
{
    public class RabbitMQClientHandler
    {
        ...
        private static string defaultRabbitMQPassword = ConfigurationManager.AppSettings["RabbitMQPassword"].Trim();
        private static string defaultRabbitVirtualHost = ConfigurationManager.AppSettings["RabbitMQVirtualHost"].Trim();
        ...
        static Main.RabbitmqMessageCallback doWork;
        static Main.RabbitmqConnectCallback rabbitmqConnectCallback;
        //
        ...
        /// </summary>
        /// <returns></returns>
        public static RabbitMQClientHandler GetInstance(Main.RabbitmqMessageCallback d, Main.RabbitmqConnectCallback rbCon)
        {
            doWork = d;
        ...
        static void Connect()
        {
            try
            {
                ...
                rabbitmqConnectCallback("[获取RabbitMQ服务器参数：" + defaultRabbitMQHostName + ":" + defaultRabbitMQPort + " (" + defaultRabbitMQUserName + "/" + defaultRabbitMQPassword + "), 正在连接中...]");
                //连接工厂
                ...
                {
                    //重新监控消息
                    RabbitmqMessageConsume(MC_SyncDataConsume, doWork);
                }

                rabbitmqConnectCallback("[尝试连接至RabbitMQ服务器：" + defaultRabbitMQHostName+"]");
            ...
            try
            {
                ...
                if (SendChannel != null && SendChannel.IsOpen)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        rabbitmqConnectCallback("[RabbitMQ重新连接，正在尝试关闭之前的Channel[发送]，但遇到错误，等待重新连接...]");
                    }
                    SendChannel = null;
                }
                ...
                if (ListenChannel != null && ListenChannel.IsOpen)
                {
                    ...
                    {
                        rabbitmqConnectCallback("[RabbitMQ重新连接，正在尝试关闭之前的Channel[接收]，但遇到错误，等待重新连接...]");
                ...
                if (Connection != null && Connection.IsOpen)
                {
                    ...
                    {
                        rabbitmqConnectCallback("[RabbitMQ重新连接，正在尝试关闭之前的连接，但遇到错误，等待重新连接...]");
        ...
        private static void Connection_ConnectionShutdown(object sender, ShutdownEventArgs e)
        {
            ...
            rabbitmqConnectCallback("[):  RabbitMQ已经断开连接，正在尝试重新连接至RabbitMQ服务器...]");
            Reconnect();
        }
        ...
        public void SyncDataFromServer(string queueName)
        {
            ...
            {
                LogHelper.WriteLog("TopicExchangeConsumeMessageFromServer运行错误：" + ex.Message, ex);
                throw ex;
            }
        }

        private static void RabbitmqMessageConsume(string queueName, Main.RabbitmqMessageCallback doWork)
        {
            try
            {
                if (Connection == null || !Connection.IsOpen) throw new Exception("连接为空或连接已经关闭");
